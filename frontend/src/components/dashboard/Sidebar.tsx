...
const getIcon = (iconName: string) => {
  switch (iconName) {
    case "LayoutDashboard": return <LayoutDashboard className="w-5 h-5 group-data-[collapsible=icon]:w-5 group-data-[collapsible=icon]:h-5" />;
    case "Bot": return <Bot className="w-5 h-5 group-data-[collapsible=icon]:w-5 group-data-[collapsible=icon]:h-5" />;
    case "Users": return <Users className="w-5 h-5 group-data-[collapsible=icon]:w-5 group-data-[collapsible=icon]:h-5" />;
    case "Route": return <Route className="w-5 h-5 group-data-[collapsible=icon]:w-5 group-data-[collapsible=icon]:h-5" />;
    case "Truck": return <Truck className="w-5 h-5 group-data-[collapsible=icon]:w-5 group-data-[collapsible=icon]:h-5" />;
    case "AlertTriangle": return <AlertTriangle className="w-5 h-5 group-data-[collapsible=icon]:w-5 group-data-[collapsible=icon]:h-5" />;
    case "BarChart3": return <BarChart3 className="w-5 h-5 group-data-[collapsible=icon]:w-5 group-data-[collapsible=icon]:h-5" />;
    case "Settings": return <Settings className="w-5 h-5 group-data-[collapsible=icon]:w-5 group-data-[collapsible=icon]:h-5" />;
    default: return <LayoutDashboard className="w-5 h-5 group-data-[collapsible=icon]:w-5 group-data-[collapsible=icon]:h-5" />;
  }
};
...
export function Sidebar() {
  ...
  return (
    <ShadcnSidebar variant="sidebar" className="border-r border-border bg-background" data-purpose="dashboard-sidebar">
              ...
              {filteredLinks.map((link, i) => {
                ...
                return (
                  <SidebarMenuItem key={i}>
                    <SidebarMenuButton 
                      render={<Link href={link.href} className="flex items-center gap-3">{getIcon(link.icon)}<span className="text-sm">{link.label}</span></Link>}
                      isActive={isActive} 
                      tooltip={link.label}

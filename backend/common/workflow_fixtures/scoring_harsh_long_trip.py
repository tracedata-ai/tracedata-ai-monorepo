...
Every packet gets a **fresh** ``event_id`` / ``device_event_id`` (UUID-based via ``new_ids``) so
split ``--segments`` runs do not collide in ``pipeline_events``. All packets share the same
``trip_id`` (default matches ``docs/03-agents/0_input_data.md``; override with ``play_workflow --trip-id``).
...
def build_events(
    *,
    ...
    Each emitted packet gets unique ``event_id`` / ``device_event_id``; ``trip_id`` is shared.
    ...
    for kind, off, idx in _TIMELINE:
        ...
        ev_id, dev_id = new_ids(kind)
        bat = f"BAT-{uuid.uuid4().hex[:12]}"

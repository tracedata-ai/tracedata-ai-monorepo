...
class PIIScrubber:
    ...
    def anonymise_driver_id(self, real_driver_id: str) -> str:
        ...
        The real_driver_id is NOT stored here — caller must retain it
        separately if needed for Postgres audit write.
        """
        if real_driver_id not in self._cache:
            if len(self._cache) >= _CACHE_MAX:
                # Evict oldest entry — simple FIFO via dict insertion order
    ...
    def scrub_location(
        ...
        Returns (None, None) if input is None.
        """
    ...
    def scrub_details(
        ...
        injury_severity_estimate is only needed by Safety Agent.

        Uses deepcopy to avoid mutating the original payload — event
        details can contain nested dicts (e.g. collision sub_system data).
        """
        if details is None:
            return None

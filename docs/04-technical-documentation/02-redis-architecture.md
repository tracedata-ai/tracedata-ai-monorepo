...
flowchart TD
    ...
    subgraph PIPELINE["Role 1 — Two-Stage Pipeline"]
        RAW[("telemetry:truck:buffer\nZSet · raw TelemetryPacket\nIngestion Tool reads")]
        PROC[("telemetry:truck:processed\nZSet · clean TripEvent\nOrchestrator reads")]
        DLQ[("telemetry:truck:rejected\nZSet · DLQ · TTL 48h\nfleet admin inspects")]
    end

    subgraph IT_BOX["Ingestion Tool (independent worker)"]
        IT["validate · scan · PII scrub\ndedup · DB WRITE 1"]
    end

    subgraph CACHE["Role 2 — Agent Working Cache"]
        CTX[("trip:trip_id:context\nString JSON · TTL-aware\n48h CRITICAL/HIGH · 10min LOW")]
        SML[("trip:trip_id:smoothness_logs\nList · TTL-aware")]
        OUT[("trip:trip_id:agent_output\nString JSON · TTL-aware")]
    end

    subgraph SIGNAL["Role 3 — Completion Signalling"]
        PUB[("trip:trip_id:events\nPub/Sub + List · TTL-aware")]
    end

    subgraph AGENTS["Agents (LangGraph workers)"]
        ORC["Orchestrator\nLangGraph"]
        SCO["Scoring Agent\nXGBoost · SHAP · AIF360"]
        SAF["Safety Agent"]
        DSP["DSP Agent\ncoaching · flagged_events"]
        SMA["Sentiment Agent\nRAG · pgvector"]
    end

    subgraph TOOLS["Tool Registry — via Intent Gate"]
        TR["context · ML/XAI · LLM\nweather · maps · LTA\nXGBoost · SHAP · Claude/GPT-4o"]
    end

    PG[("Postgres + pgvector\nSource of Truth")]
    LS["LangSmith\nobservability · traces"]

    DEV -->|zadd score=priority| RAW
    APP -->|zadd score=priority| RAW
    RAW -->|zpopmin| IT
    IT -->|success: zadd| PROC
    IT -->|rejected: zadd TTL 48h| DLQ
    IT -->|DB WRITE 1| PG

    PROC -->|zpopmin| ORC
    ORC -->|warm cache + flagged_events| CTX
    ORC -->|read history| PG

    SCO -->|lpush per batch| SML
    SCO & SAF & DSP & SMA -->|store result| OUT
    SCO & SAF & DSP & SMA -->|tool calls via Intent Gate| TOOLS

    SCO & DSP -->|hydrate| CTX
    SCO -->|lrange all| SML
    DSP -->|get scoring output| OUT

    SCO & SAF & DSP & SMA -->|publish CompletionEvent| PUB

...
export function LandingBadge({ text, statusColor = "bg-brand-teal", className = "" }: LandingBadgeProps) {
  return (
    <div className={`inline-flex items-center gap-3 px-4 py-1.5 rounded-full bg-white/80 dark:bg-white/5 border border-black/10 dark:border-white/10 glass-card shadow-sm ${className}`}>
      ...
      <span className="text-[10px] font-black text-foreground dark:text-white uppercase tracking-[0.2em] opacity-80">
        {text}
      </span>
    </div>
  );

...
interface TooltipProps { active?: boolean; payload?: { value: number | string; payload: Record<string, unknown> }[]; label?: string }
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white/90 border border-slate-200 p-4 rounded-xl shadow-xl backdrop-blur-md">
        ...
        <p className="text-[10px] text-slate-500 font-bold mt-2 uppercase">Trip ID: {String(payload[0].payload.tripId)}</p>
      </div>
    );
  }
  return null;
              ...
              >
                <div className="space-y-6">
                  <div className="text-xl font-medium text-slate-900 leading-relaxed italic border-l-4 border-brand-blue/20 pl-6 py-1">
                    &ldquo;{driver.explanation.humanSummary}&rdquo;
                  </div>
